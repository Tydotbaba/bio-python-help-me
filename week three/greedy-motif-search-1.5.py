...
def Count(motifs):
	...
	if len(motifs) == 1:
		return {
			'A': [1 if 'A' == i else 0 for i in motifs[0]],
			'C': [1 if 'C' == i else 0 for i in motifs[0]],
			'G': [1 if 'G' == i else 0 for i in motifs[0]],
			'T': [1 if 'T' == i else 0 for i in motifs[0]],
		}
	for j in range(len(motifs[0])):
		# let j be the motifs column
		# get each row nueclotide in motifs column
		res = [string[j] for string in motifs]
		result['A'].append(int(res.count('A')))
		result['C'].append(int(res.count('C')))
		result['G'].append(int(res.count('G')))
		result['T'].append(int(res.count('T')))
	# return the sum of the result
	# print(result)
	return result
...
# code to parse input file for GreedyMotifSearch
def parseFile(f):
	res = []
...
# change your code below this line for GreedyMotifSearch
...
try:
	...
	raise e
# change your code above this line for GreedyMotifSearch
